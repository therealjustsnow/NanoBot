...
_SCHEMA = {
    ...
    "topgg_v1_token":      (str, False, "top.gg v1 API token for commands sync (str or null)"),
    ...
    "vote_webhook_secret": (str, False, "the secret used by bot list sites to verify webhooks"),
}
...
def validate(cfg: dict) -> list[ConfigIssue]:
    ...
    if not isinstance(prefix, str) or not prefix:
        ...
        issues.append(ConfigIssue("default_prefix", f"'{prefix}' contains a space — prefixes can't have spaces", True))
    elif len(prefix) > 5:
        issues.append(ConfigIssue("default_prefix", f"'{prefix}' is {len(prefix)} chars — max is 5", True))

    # ── log_level ──────────────────────────────────────────────────────────────
    raw_level = cfg.get("log_level", "INFO")
    if not isinstance(raw_level, str):
        issues.append(ConfigIssue("log_level", f"Must be a string, got {type(raw_level).__name__}", False))
