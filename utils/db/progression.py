...
- ``achievements_earned`` — one row per (guild, user, achievement key) the
...
- ``weekly_objectives`` — up to three deterministically-picked objectives per
  (guild, user, period). ``week`` is really just a period key (an ISO week
  ...
  read.
- ``progression`` — one row per (guild, user): prestige rank, plus the
...
# ── Achievements ───────────────────────────────────────────────────────────────
async def get_earned_achievements(guild_id: int, user_id: int) -> dict[str, float]:
    """Earned achievement keys → their earned_at epoch, for this member."""
    async with _conn().execute(
        "SELECT key, earned_at FROM achievements_earned WHERE guild_id=? AND user_id=?",
...
async def try_award_achievement(
    guild_id: int, user_id: int, key: str, *, earned_at: float | None = None
) -> bool:
    ...
    cur = await _conn().execute(
        "INSERT OR IGNORE INTO achievements_earned (guild_id, user_id, key, earned_at) "
        "VALUES (?,?,?,?)",
        (
            str(guild_id),
            str(user_id),
...
async def get_period_objectives(guild_id: int, user_id: int, period: str) -> list[dict]:
    async with _conn().execute(
        "SELECT obj_key, target, baseline, claimed FROM weekly_objectives "
        "WHERE guild_id=? AND user_id=? AND week=? ORDER BY obj_key ASC",
...
async def get_or_create_objective(
    ...
    (guild, user, period, obj_key) with a different target/baseline is a
    no-op; the row this period already exists."""
    gid, uid = str(guild_id), str(user_id)
    await _conn().execute(
        "INSERT OR IGNORE INTO weekly_objectives "
        "(guild_id, user_id, week, obj_key, target, baseline, claimed) "
        "VALUES (?,?,?,?,?,?,0)",
        (gid, uid, period, obj_key, float(target), float(baseline)),
    )
    await _conn().commit()
    async with _conn().execute(
        "SELECT obj_key, target, baseline, claimed FROM weekly_objectives "
        "WHERE guild_id=? AND user_id=? AND week=? AND obj_key=?",
        (gid, uid, period, obj_key),
    ) as cur:
...
async def try_claim_objective(
    guild_id: int, user_id: int, period: str, obj_key: str, current_value: float
) -> bool:
    ...
    cur = await _conn().execute(
        ...
        "WHERE guild_id=? AND user_id=? AND week=? AND obj_key=? AND claimed=0 "
        "AND (? - baseline) >= target",
        (str(guild_id), str(user_id), period, obj_key, float(current_value)),
    )
    await _conn().commit()
    return cur.rowcount > 0
...
# ── Prestige / profile ──────────────────────────────────────────────────────────
async def get_progression(guild_id: int, user_id: int) -> dict:
    async with _conn().execute(
        "SELECT prestige, selected_title FROM progression WHERE guild_id=? AND user_id=?",
        (str(guild_id), str(user_id)),
    ) as cur:
        row = await cur.fetchone()
    if row:
...
async def try_advance_prestige(
    guild_id: int, user_id: int, new_rank: int, *, expected: int
) -> bool:
    ...
    if expected == 0:
        ...
        await _conn().execute(
            "INSERT OR IGNORE INTO progression (guild_id, user_id) VALUES (?,?)",
            (str(guild_id), str(user_id)),
        )
    cur = await _conn().execute(
        "UPDATE progression SET prestige=? "
        "WHERE guild_id=? AND user_id=? AND prestige=?",
        (int(new_rank), str(guild_id), str(user_id), int(expected)),
    )
    await _conn().commit()
    return cur.rowcount > 0


async def set_selected_title(guild_id: int, user_id: int, title: str) -> None:
    """Set (or, with an empty string, clear) the member's displayed title."""
    await _conn().execute(
        "INSERT INTO progression (guild_id, user_id, selected_title) VALUES (?,?,?) "
        "ON CONFLICT(guild_id, user_id) DO UPDATE SET "
        "selected_title=excluded.selected_title",
        (str(guild_id), str(user_id), title),
    )
    await _conn().commit()
