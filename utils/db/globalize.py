...
async def drop_per_guild_coin_faucets(conn):
    ...
    await conn.execute(
        ...
        # Added after this migration (the global level-up channel, then the
        # global-XP switch), and listed here for the same reason as
        # activities_stats' co-op columns: this rebuild REPLACES the table, so
        # without them a database migrating for the first time comes out
        # missing what _ensure_columns just added — and every read of
        # level_config fails for that whole run, because get_level_config names
        # its columns. Not in the INSERT below — the source table predates
        # them, so every guild starts on the defaults (announce, following
        # /level announce; global XP on).
