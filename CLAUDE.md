...
- `tests/test_birthday_helpers.py` — pure date helpers from `cogs/birthday.py` (`parse_birthday`, `next_birthday_date`, `days_until_birthday`, `is_birthday_today` incl. leap-day, `age_on`, `fmt_birthday`) + the FFmpeg song-command builder
...
| `birthday.py` | Per-guild birthday tracker. `/birthday` group: members self-register (`set`/`remove`/`view`/`list`) with optional birth year (drives the age line); per-guild setup (Manage Server) `channel`/`disable`/`timezone`/`hour`/`message`/`gifs`/`voice`/`ping`/`config`/`test`. A `tasks.loop(minutes=15)` background check (not an event — restart-safe) announces each birthday once per year in the configured channel with a random festive GIF, gated on the guild-local timezone + hour. The per-row `last_announced` local-date stamp is written **before** the announcement, so a crash can't double-fire and a later same-day check is a no-op (offline during the hour → still fires that local day, no cross-day catch-up). If the birthday member is in a voice channel at announce time, the bot joins, plays a synthesized "Happy Birthday" (sine tones rendered once by FFmpeg to `data/birthday_cache/`, or a `song` path/URL override), and leaves — exactly once (guarded by an in-flight `_singing` set + the announced stamp; skipped if a voice client already exists so it won't fight the music cog). Voice playback needs FFmpeg + PyNaCl; it degrades gracefully (text-only) when unavailable. |
