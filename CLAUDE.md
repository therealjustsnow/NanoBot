...
| `views.py` | `TeamupView` (/teamup multi-teammate co-op confirm) + `RaidView` (/raid join board); they call back into the cog via a `TYPE_CHECKING` `Economy` hint. |
| `cog.py` | The `Economy` cog: full command surface (`/balance`, `/daily`, `/pay`, `/teamup`, `/raid`, `/coin …`, `/shop …`). Carries the full module docstring (commands enumerated for `test_docs_freshness`). |
