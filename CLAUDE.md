...
| `music.py` | Voice music player: yt-dlp streaming, Spotify link support (no API key — embed-page metadata scraped then matched on YouTube at play time), per-guild queue, interactive Now Playing card (buttons), search picker, vote-skip, playnext/playnow/stream/shuffleplay, follow, move/jump, loop/shuffle/seek/speed/audio-filters/volume, lyrics, grab, pldump, autoplay + persistent autoplaylist (add accepts whole playlists), 24/7 stay-connected mode (`radio`/`247`, per-guild, off by default), idle auto-disconnect. Audio is sent as Opus — stream-copied from the source when unprocessed, else re-encoded with libopus at the voice channel's bitrate. Reads `[music]` config (incl. cookies). Requires FFmpeg + PyNaCl. Playback state is in-memory (not restart-safe); the autoplaylist and 24/7 setting persist in SQLite. |
