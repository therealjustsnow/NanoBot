...
- `tests/test_casino_commands.py` — `/casino` bet flows (debit-before-resolve, limits, insufficient funds), toggle-off behavior, admin permission enforcement
...
| `casino/` | Casino games — the gambling overhaul (a package: constants/helpers/views/cog). One `/casino` hybrid group (bare = overview of games, limits, jackpot, your stats): `flip <bet> <side>` (1.92x), `dice <bet>` (2d6 vs dealer, 2.1x, tie pushes), `slots <bet>` (reel pairs/triples, cash RTP ≈95%; triple 7️⃣ also claims the progressive jackpot), `roulette <bet> <space>` (European single-0 wheel; outside bets 2x, straight number 35x), `blackjack <bet>` (interactive Hit/Stand buttons, 4-deck shoe, dealer stands 17, natural pays 3:2; pure hand/dealer logic tested Discord-free; bet debited upfront, view auto-stands on timeout), `jackpot`, `stats [member]`, `top [page]` (net winnings), plus Manage-Server `limit <min> <max>`/`toggle`/`config`. Every bet is debited before resolving via `try_debit_coins`; payouts via `add_coins`; a 3+ win streak adds +5%/step payout capped +25%; 20% of every net loss feeds the guild's progressive jackpot (`casino_config.jackpot_pool`, claimed by an atomic compare-and-swap so two triple-7s can't both win). `record_casino_game` is a single atomic upsert (streak/biggest-win via SQL CASE/MAX). |
