...
| `music.py` | Voice music player: yt-dlp streaming, Spotify link support (no API key — embed-page metadata scraped then matched on YouTube at play time), per-guild queue, interactive Now Playing card (buttons), search picker, vote-skip, playnext/playnow/stream/shuffleplay, follow, move/jump, loop/shuffle/seek/speed/audio-filters/volume, lyrics, grab, pldump, autoplay + persistent autoplaylist (add accepts whole playlists), 24/7 stay-connected mode (`radio`/`247`, per-guild, off by default), idle auto-disconnect. Audio output is configurable via `music_use_opus`: Opus (default) is stream-copied from the source when unprocessed, else re-encoded with libopus at the voice channel's bitrate; PCM mode decodes every track and lets discord.py encode, trading CPU for instant (re-stream-free) volume changes. Optional `music_persist_queue` saves the queue to SQLite and resumes it (rejoins the last voice channel) on restart; optional `music_predownload` fetches the next queued track to `data/music_cache/` while one plays for gapless playback. Reads `[music]` config (incl. cookies). Requires FFmpeg + PyNaCl. Live playback position is in-memory; the autoplaylist, 24/7 setting, and (when enabled) the queue persist in SQLite. |
