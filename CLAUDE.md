...
| Module | Holds |
|---|---|
| `constants.py` | `BIRTHDAY_COLOR`, the default announcement message + `_VARS_HELP`, the `_BIRTHDAY_GIFS` pool, the `_TZ_CHOICES` dropdown list + `_REGION_TZ` map, the month tables (`_MONTH_NAMES`/`_MONTHS`/`_MAX_DAY`), and the `_HB_NOTES` melody + `_SONG_DIR`/`_SONG_PATH`. |
...
| `views.py` | The `/birthday timezone` dropdown (`TimezoneView` + `_TimezoneSelect`); calls back into the cog via a `TYPE_CHECKING` `Birthday` hint. |
| `cog.py` | The `Birthday` cog: self-register/setup commands, the 15-minute announce loop, and the `on_voice_state_update` song trigger. Carries the full module docstring (commands enumerated for `test_docs_freshness`). |
