...
- `tests/test_activities_helpers.py` — pure activity helpers from `cogs/activities/` (work pay + career ladder, ore/hunt/explore outcome distributions summing to 1, luck-shifted pickaxe odds, rob success/steal/fine math)
- `tests/test_activities_db.py` — activities accessors in `utils/db/` against in-memory SQLite (config roundtrip, the generic atomic `try_claim_activity` cooldown claim incl. the shared-row fresh-activity edge, the conditional-UPDATE pickaxe-upgrade race guard)
...
- `tests/test_inventory_commands.py` — `/inventory` view/use/sell/give/chest flows incl. the audit regressions: bulk-use effect-duration cap (only the clamped qty is consumed) and self-gift rejection
