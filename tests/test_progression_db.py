...
async def test_no_achievements_earned_by_default():
    assert await db.get_earned_achievements(A) == {}


async def test_award_achievement_first_time_succeeds():
    awarded = await db.try_award_achievement(A, "fish_caught_10", earned_at=1000.0)
...
async def test_award_achievement_is_one_time_only():
    ...
    first = await db.try_award_achievement(A, "fish_caught_10", earned_at=1000.0)
    second = await db.try_award_achievement(A, "fish_caught_10", earned_at=2000.0)
...
async def test_achievements_are_per_user_and_follow_them_everywhere():
    """Achievements are global: earned once, owned by the user in every server
    (they used to be re-earnable per guild)."""
    await db.try_award_achievement(A, "fish_caught_10")
    assert "fish_caught_10" in await db.get_earned_achievements(A)
    assert await db.get_earned_achievements(B) == {}
    # Re-awarding the same key (from any server) never lands twice.
...
async def test_get_or_create_objective_creates_with_given_baseline():
    row = await db.get_or_create_objective(A, "2026-W30", "weekly_fish_catch", 20, 5)
    assert row == {
...
async def test_get_or_create_objective_is_idempotent_baseline_sticks():
    ...
    first = await db.get_or_create_objective(A, "2026-W30", "weekly_fish_catch", 20, 5)
    second = await db.get_or_create_objective(
        A, "2026-W30", "weekly_fish_catch", 999, 999
    )
    assert second == first


async def test_get_period_objectives_lists_rows_for_that_period():
    await db.get_or_create_objective(A, "2026-W30", "weekly_fish_catch", 20, 5)
    await db.get_or_create_objective(A, "2026-W30", "weekly_work", 5, 0)
    await db.get_or_create_objective(A, "2026-W31", "weekly_work", 5, 0)
...
async def test_try_claim_objective_fails_when_not_yet_complete():
    ...
    claimed = await db.try_claim_objective(A, "2026-W30", "weekly_fish_catch", 10)
...
async def test_try_claim_objective_succeeds_when_complete():
    ...
    claimed = await db.try_claim_objective(A, "2026-W30", "weekly_fish_catch", 25)
...
async def test_try_claim_objective_is_one_time_only():
    await db.get_or_create_objective(A, "2026-W30", "weekly_fish_catch", 20, 5)
    first = await db.try_claim_objective(A, "2026-W30", "weekly_fish_catch", 25)
    second = await db.try_claim_objective(A, "2026-W30", "weekly_fish_catch", 25)
    assert first is True
    assert second is False


async def test_try_claim_objective_missing_row_is_a_noop():
    claimed = await db.try_claim_objective(A, "2026-W30", "nonexistent", 100)
    assert claimed is False


# ── Prestige / profile ────────────────────────────────────────────────────────────
async def test_get_progression_defaults():
    assert await db.get_progression(A) == {"prestige": 0, "selected_title": ""}


async def test_try_advance_prestige_from_zero_succeeds():
    advanced = await db.try_advance_prestige(A, 1, expected=0)
    assert advanced is True
    assert (await db.get_progression(A))["prestige"] == 1
...
async def test_try_advance_prestige_cas_blocks_stale_expected():
    ...
    await db.try_advance_prestige(A, 1, expected=0)
    # A racing confirm that also computed expected=0 (stale read) must lose.
    second = await db.try_advance_prestige(A, 1, expected=0)
    assert second is False
    assert (await db.get_progression(A))["prestige"] == 1


async def test_try_advance_prestige_sequential_ranks():
    assert await db.try_advance_prestige(A, 1, expected=0) is True
    assert await db.try_advance_prestige(A, 2, expected=1) is True
    assert (await db.get_progression(A))["prestige"] == 2


async def test_prestige_is_per_user_and_shared_across_servers():
    await db.try_advance_prestige(A, 1, expected=0)
    assert (await db.get_progression(A))["prestige"] == 1
    assert (await db.get_progression(B))["prestige"] == 0


async def test_set_selected_title_roundtrip():
    await db.set_selected_title(A, "Master Angler")
    assert (await db.get_progression(A))["selected_title"] == "Master Angler"
    await db.set_selected_title(A, "")
    assert (await db.get_progression(A))["selected_title"] == ""
...
async def test_set_selected_title_before_any_prestige_row_exists():
    ...
    await db.set_selected_title(B, "Tycoon")
    row = await db.get_progression(B)
