...
async def test_stats_defaults():
    stats = await db.get_activity_stats(G, A)
    assert stats == {
        "last_work": 0.0,
        "work_shifts": 0,
        "last_mine": 0.0,
...
async def test_unknown_activity_rejected():
    with pytest.raises(ValueError):
        await db.try_claim_activity(G, A, "fishing", 1000.0, 60)


async def test_first_claim_succeeds_and_counts():
    assert await db.try_claim_activity(G, A, "work", 1000.0, 3600) == 0
    stats = await db.get_activity_stats(G, A)
...
async def test_claim_blocked_inside_cooldown():
    await db.try_claim_activity(G, A, "mine", 1000.0, 1800)
    retry = await db.try_claim_activity(G, A, "mine", 1500.0, 1800)
    assert retry == 1300
    stats = await db.get_activity_stats(G, A)
...
async def test_claim_allowed_after_cooldown():
    await db.try_claim_activity(G, A, "hunt", 1000.0, 2700)
    assert await db.try_claim_activity(G, A, "hunt", 3700.0, 2700) == 0
    stats = await db.get_activity_stats(G, A)
    assert stats["hunt_count"] == 2


async def test_claim_retry_is_at_least_one_second():
    await db.try_claim_activity(G, A, "explore", 1000.0, 10800)
    retry = await db.try_claim_activity(G, A, "explore", 10799.5, 10800)
    assert retry >= 1
...
async def test_different_activities_have_independent_cooldowns():
    ...
    assert await db.try_claim_activity(G, A, "mine", 1000.0, 1800) == 0
    stats = await db.get_activity_stats(G, A)
    assert stats["work_shifts"] == 1
    assert stats["mine_count"] == 1
...
async def test_different_users_have_independent_cooldowns():
    await db.try_claim_activity(G, A, "rob", 1000.0, 14400)
    assert await db.try_claim_activity(G, B, "rob", 1000.0, 14400) == 0


async def test_different_guilds_have_independent_cooldowns():
    await db.try_claim_activity(G, A, "rob", 1000.0, 14400)
    assert await db.try_claim_activity(999, A, "rob", 1000.0, 14400) == 0


# ── Pickaxe upgrades (race guard + refund contract) ──────────────────────────────
async def test_pickaxe_upgrade_from_fresh_row():
    assert await db.set_pickaxe_level(G, A, 1, expected=0) is True
    assert (await db.get_activity_stats(G, A))["pickaxe_level"] == 1
...
async def test_pickaxe_upgrade_race_second_buyer_loses():
    ...
    assert await db.set_pickaxe_level(G, A, 1, expected=0) is False
    assert (await db.get_activity_stats(G, A))["pickaxe_level"] == 1
    assert await db.set_pickaxe_level(G, A, 2, expected=1) is True


async def test_pickaxe_upgrade_does_not_touch_other_users():
    await db.set_pickaxe_level(G, A, 1, expected=0)
    assert (await db.get_activity_stats(G, B))["pickaxe_level"] == 0
