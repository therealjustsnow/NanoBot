...
async def test_use_timed_effect_grants_and_consumes(bot):
    ...
    author = config().members[0]
    await db.add_item(guild.id, author.id, "lucky_charm", 2)

    await dpytest.message("!inventory use lucky_charm", member=author)
    sent = dpytest.get_message()
    assert "luck" in sent.embeds[0].description
    assert await db.get_item_qty(guild.id, author.id, "lucky_charm") == 1
    effs = await db.get_active_effects(guild.id, author.id)
...
async def test_use_bulk_timed_effect_capped_at_duration_ceiling(bot):
    ...
    guild = config().guilds[0]
    author = config().members[0]
    await db.add_item(guild.id, author.id, "lucky_charm", 1000)
    ...
    assert (
        await db.get_item_qty(guild.id, author.id, "lucky_charm")
        == 1000 - expected_used
    )
...
async def test_give_rejects_self(bot):
    guild = config().guilds[0]
    author = config().members[0]
    await db.add_item(guild.id, author.id, "lucky_charm", 1)
...
async def test_give_transfers_between_members(bot):
    ...
    await db.add_item(guild.id, author.id, "lucky_charm", 3)
    await dpytest.message(
        f"!inventory give {other.mention} lucky_charm 2", member=author
    )
    dpytest.get_message()
    assert await db.get_item_qty(guild.id, author.id, "lucky_charm") == 1
    assert await db.get_item_qty(guild.id, other.id, "lucky_charm") == 2
...
async def test_sell_unsellable_item_refused(bot):
    guild = config().guilds[0]
    author = config().members[0]
    await db.add_item(guild.id, author.id, "treasure_key", 1)
...
async def test_chest_requires_key(bot):
    guild = config().guilds[0]
    author = config().members[0]
    await db.add_item(guild.id, author.id, "treasure_chest", 1)
    await dpytest.message("!inventory use treasure_chest", member=author)
    sent = dpytest.get_message()
    assert "Key" in sent.embeds[0].description
    assert await db.get_item_qty(guild.id, author.id, "treasure_chest") == 1
...
async def test_chest_opens_with_key_and_pays(bot):
    ...
    assert await db.get_item_qty(guild.id, author.id, "treasure_chest") == 0
    assert await db.get_item_qty(guild.id, author.id, "treasure_key") == 0
    assert await db.get_balance(guild.id, author.id) >= 250
