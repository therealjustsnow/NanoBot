...
async def test_inventory_starts_empty():
    assert await db.get_inventory(G, A) == []
    assert await db.get_item_qty(G, A, "worm") == 0


async def test_add_and_stack_items():
    assert await db.add_item(G, A, "worm", 3) == 3
    assert await db.add_item(G, A, "worm", 2) == 5
    await db.add_item(G, A, "iron_ore", 1)
    inv = await db.get_inventory(G, A)
    assert {s["item_key"]: s["qty"] for s in inv} == {"worm": 5, "iron_ore": 1}


async def test_consume_within_stock():
    await db.add_item(G, A, "worm", 3)
    assert await db.try_consume_item(G, A, "worm", 2) is True
    assert await db.get_item_qty(G, A, "worm") == 1


async def test_consume_over_stock_fails_and_keeps_qty():
    await db.add_item(G, A, "worm", 1)
    assert await db.try_consume_item(G, A, "worm", 2) is False
    assert await db.try_consume_item(G, A, "worm", 0) is False
    assert await db.try_consume_item(G, B, "worm", 1) is False
    assert await db.get_item_qty(G, A, "worm") == 1


async def test_zero_qty_stacks_hidden():
    await db.add_item(G, A, "worm", 1)
    await db.try_consume_item(G, A, "worm", 1)
    assert await db.get_inventory(G, A) == []


async def test_transfer_item():
    await db.add_item(G, A, "worm", 2)
    assert await db.transfer_item(G, A, B, "worm", 2) is True
    assert await db.get_item_qty(G, A, "worm") == 0
    assert await db.get_item_qty(G, B, "worm") == 2
    assert await db.transfer_item(G, A, B, "worm", 1) is False


# ── Effects ────────────────────────────────────────────────────────────────────
async def test_timed_effect_lifecycle():
    await db.grant_effect(G, A, "luck", 0.5, duration=60, now=1000.0)
    effs = await db.get_active_effects(G, A, now=1030.0)
    assert effs["luck"]["magnitude"] == 0.5
    assert effs["luck"]["expires_at"] == 1060.0
    # Expired → pruned on read.
    assert await db.get_active_effects(G, A, now=1060.0) == {}


async def test_regrant_replaces_effect():
    await db.grant_effect(G, A, "luck", 0.5, duration=60, now=1000.0)
    await db.grant_effect(G, A, "luck", 0.2, duration=600, now=1010.0)
    effs = await db.get_active_effects(G, A, now=1030.0)
...
async def test_charge_effect_consumes_to_deletion():
    await db.grant_effect(G, A, "bait", 1.0, uses=2)
    assert await db.consume_effect_use(G, A, "bait") is True
    effs = await db.get_active_effects(G, A)
    assert effs["bait"]["uses_left"] == 1
    assert await db.consume_effect_use(G, A, "bait") is True
    assert await db.get_active_effects(G, A) == {}
    assert await db.consume_effect_use(G, A, "bait") is False


async def test_clear_effect():
    await db.grant_effect(G, A, "luck", 0.5, duration=60, now=1000.0)
    await db.clear_effect(G, A, "luck")
    assert await db.get_active_effects(G, A, now=1000.0) == {}
