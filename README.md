...
| `topgg_v1_token` | top.gg v1 API token — enables stat posting, command sync, and vote webhooks |
| `dbl_token` | discordbotlist.com bot token — enables stat posting, command sync, and vote webhooks |
| `discordbotsgg_token` | discord.bots.gg bot token — enables stat posting and vote webhooks |
| `vote_webhook_port` | Port for the vote webhook HTTP server (default `5000`) |
| `vote_webhook_secret` | Shared secret for webhook verification. top.gg uses HMAC-SHA256; DBL and discord.bots.gg use a plain `Authorization` header match |
