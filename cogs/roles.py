...
class Roles(commands.Cog):
    ...
    async def colorgen(
        ...
        # ── 1. Create roles ───────────────────────────────────────────────────
        # Discord rate-limits role creation to ~1/s sustained.
        # 0.3s was too aggressive — caused 503 "overflow" on a full palette.
        # 1.0s per role is safe; the 15-minute followup window is more than enough.
        _CREATE_DELAY = 1.0
        _MAX_RETRIES = 2
        ...
        # Brief pause after bulk creation before hitting the positions endpoint.
        ...
        try:
            ...
            log.warning(f"colorgen: could not reposition roles: {exc}")
            positioned = False
        ...
        if positioned:
            ...
            lines.append(
                "⚠️ Could not auto-position roles — move them above your regular member "
                "roles manually so the colours show in the member list."
            )
        if skipped:
        ...
        log.info(
            ...
            f"by {interaction.user} — positioned: {positioned}"
        )
